...
import { GetPatternRequest, SetPatternRequest, SetPatternsRequest, SetAnimationsRequest, SetMoodRequest } from '../../api/requests';
...
const useCeiledAPI = (): [Map<number, Pattern | Animation>, CeiledAPI] => {
  ...
  // GET pattern
  const getPattern = useCallback((channel: number | 'all'): Promise<void> => {
  ...
  // SET pattern
  const updatePattern = useCallback((channel: number | 'all', pattern: Pattern): Promise<void> => {
    if (isConnected(socket)) {
      if (channel === 'all') {
  ...
  // SET patterns
  const setPatterns = useCallback((patterns: Map<number, Pattern>): Promise<void> => {
    if (isConnected(socket)) {
      setState(patterns);
  ...
  // SET animations
  const setAnimations = useCallback((animations: Map<number, Animation>): Promise<void> => {
    if (isConnected(socket)) {
      setState(animations);
  ...
  // SET mood
  const setMood = useCallback((mood: Moods): Promise<void> => {
    if (isConnected(socket)) {
