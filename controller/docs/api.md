...
| `brightness`       | `number`  | Brightness of the entire pattern. Must be a value between 0 and 100 (inclusive), where 0 is complete black, and 100 is full brightness.
| `roomLight`        | `number`  | Amount of 'normal' room lighting to mix in with the colours used in the pattern. Should be a value between 0 and 100, where 0 means the colours are kept the way they are sent, and 100 means that the colours are fully replaced by room lighting.
